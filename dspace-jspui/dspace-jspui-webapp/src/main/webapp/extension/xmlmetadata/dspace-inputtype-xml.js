...
function XMLMetadata(inputID, oFCKeditorBasePath)
{
	...
	oXML.createRootForm = function() {
		...
		if (jQuery.trim(inputXML) == "") return;
...
// XMLRetrieve ver 1.0
// Complete at 2008/10/9
function XMLRetrieve(context)
{
	//var oXML = this;
	var oXML = new Object;
	...
	else if (typeof(context) == "string" && jQuery.trim(context).substr(0, 1) == "<")
		oXML.context = jQuery(context);
	else if (typeof(context) == "string" && jQuery.trim(context).substr(0, 1) == "#")
	{
		oXML.context = jQuery(jQuery(context).val());
		if (oXML.context.children(".node").length == 1)
			oXML.context = oXML.context.children(".node");
	}
	
	oXML.length = 0;
	
	oXML.importById = function(id)
	{
		oXML.context = jQuery(jQuery("#"+id).val());
	...
	oXML.retrieve = function(path)
	{
		if (typeof(path) == "undefined" || path == "")
		{
			//if (oXML.attr("node-type", "node"))
			return oXML.context;
		}
		var common = path.indexOf(".");
		if (common != -1)
		{
			var childPath = path.substring(0, common);
			var otherPath = path.substring(common+1, path.length);
		}
		else
		{
			var childPath = path;
		...
		var colon = childPath.lastIndexOf(":");
		if (colon == -1)
		{
			var child = childPath;
			var index = null;
		...
		{
			var index = parseInt(childPath.substring(colon+1, childPath.length));
			if (index != "NaN")
				var child = childPath.substring(0, colon);
			else
			{
				index = null;
				var child = childPath;
		...
		//filter get
		var fc = new Array;
		if (oXML.selectHasFilter(child) == true)
		{
			fc = oXML.selectFilter(child);
			child = child.substring(0, child.indexOf("["));
		}
		//filter test
			//t(fc.length + " - " + child + " : " + index);

		var oC = oXML.children(child);
		...
		for (var i = 0; i < oC.length; i++)
		{
			//filter check
			//var hasAttr = false;
			var node = XMLRetrieve(oC.eq(i));
			for (var j = 0; j < fc.length; j++)
			{
				var attr = fc[j][0];
				var value = fc[j][1];
				
				node = node.filter(attr, value);
			}
			if (node.length == 0)
				continue;
			...
			{
				if (otherPath != "")
					output.append(XMLRetrieve(oC.eq(i)).retrieve(otherPath));
				else
					output.append(oC.eq(i));
			}
	...
	oXML.attr = function(name, context)
	{
		if (typeof(context) == "object")
			return context.children("."+name).html();
		else if (typeof(context) == "string")
			return (oXML.context.children("."+name).html() == context)
		else
			return oXML.context.children("."+name).html();
	}
	oXML.title = function(comparison)
	{
		if (typeof(comparison) == "object")
		{
			var childTitle = oXML.attr("node-title", comparison);
			return oXML.title(childTitle);
		}
		else if (oXML.valid(comparison) == false)
			return oXML.attr("node-title");
		else
			return (oXML.title() == comparison);
	}
	
	oXML.children = function(title, index)
	{
		//var type = oXML.attr("node-type");
		///if (type == "node")	var class = "node-contents";
		//else if (type == "input")	var class = "input-values";
		
		//var childrenNode = oXML.context.children("."+class);
		var childrenNode = oXML.context.children(".node-contents");
		if (childrenNode.length == 0) return null;
		if (oXML.valid(title) == false)
		{
		...
		else if (typeof(title) == "number")
		{
			...
			var child = childrenNode.children(":eq("+title+")");
			if (child.length != 0)
				return child;
			else
				return null;
		...
		{
			//var type = title.children(".node-type").text();
			//if (type == "node")	var class = "node-contents";
			//else if (type == "input")	var class = "input-values";
			
			var children = title.children(".node-contents");
			var outputAry = jQuery("<div></div>");
			for (var i = 0; i < children.length; i++)
			{
				outputAry.append(children.filter(":eq("+i+")").children());
			}
			
			//return children.children();
			return outputAry.contents();
		}
		else
		{
			//t(0);
			...
			{
				//t("j: " + j);
				if (typeof(index) == "number" && j != index) continue;
				for (var i = 0; i < childrenNode.children().length; i++)
				{
					//t("j: " + j + ", i: "+i + ", index:" + index);
					
					var child = childrenNode.filter(":eq("+j+")").children(":eq("+i+")");
					//t(oXML.attr("node-title", child) + ", "+ title);
					
					if (oXML.attr("node-title", child) == title)
					{
						
						outputAry.append(child);
	...
	oXML.val = function(attr, value)
	{
		var type = oXML.attr("node-type");
		if (type == "input")
		{
			if (oXML.valid(attr) == false && oXML.valid(value) == false)
			{
				//return all values
				var values = oXML.context.children(".input-values");
				var output = new Array;
				for (var i = 0; i < values.length; i++)
				{
					var v = jQuery.trim(values.eq(i).html());
					output.push(v);
				}
				return output;
			}
			else if ((typeof(attr) == "string" && attr != "") || typeof(attr) == "number")
			{
				var valuesAry = oXML.val();
				if (jQuery.inArray(attr, valuesAry) != -1)
					return true;
				else
					return false;
			}
	...
	oXML.filter = function(attr, value)
	{
		var type = oXML.attr("node-type");
		if (type == "node")
		{
			//remove node-contents which has not title=attr
			var contents = oXML.context.children(".node-contents");
			for (var i = 0; i < contents.length; i++)
			{
				var nodes = contents.eq(i).children(".node");
				var hasAttr = false;
				for (var j = 0; j < nodes.length; j++)
				{
					var c = XMLRetrieve(nodes.eq(j));
					
					//if not input, continue
					if (c.attr("node-type", "input") == false)
						continue;
					else if (c.title(attr) == true)
					{
						if (oXML.valid(value) == true && c.val(value) == false)
							continue;
						hasAttr = true;
						continue;
					}
	...
	oXML.valid = function(argument)
	{
		if (typeof(argument) == "undefined" || (typeof(argument) == "string" && argument == ""))
			return false;
		else
			return true;
	}
	
	oXML.selectFilter = function(path, ary)
	{
		if (typeof(ary) != "array")
			var ary = new Array;
		
		var pathOther = path;
		if (oXML.selectHasFilter(path))
		{
			var attr = path.substring(path.lastIndexOf("[")+1, path.lastIndexOf("="));
			var value = path.substring(path.lastIndexOf("=")+1, path.lastIndexOf("]"));
			
			pathOther = path.substring(0, path.lastIndexOf("["));
		...
		//t(pathOther + "\n" + ary);
		if (oXML.selectHasFilter(pathOther))
		{
			ary = ary.concat(oXML.selectFilter(pathOther, ary));
			//t(ary);
		}
	...
	oXML.selectHasFilter = function(child)
	{
		child = jQuery.trim(child);
		//t([child.lastIndexOf("["), child.lastIndexOf("="), child.lastIndexOf("]"), child.length])
		if (child.lastIndexOf("[") > 0 
			&& child.lastIndexOf("=") > child.lastIndexOf("[")
			&& child.lastIndexOf("]") > child.lastIndexOf("=")
			&& child.lastIndexOf("]") == child.length - 1)
			return true;
		else
			return false;
	...
	oXML.nt = function(compairson)
	{
		if (oXML.valid(compairson) == false)
			return oXML.attr("node-type");
		else
			return (oXML.nt() == compairson)
	}
	
	oXML.eq = function(index)
	{
		if (oXML.valid(index) == false)
			return oXML;
		
		var className = oXML.ntClass();
		oXML.context.children(className).not(":eq("+index+")").remove();
	...
	oXML.html	= function()
	{
		return oXML.context.html();
	}
	
	oXML.ntClass = function()
	{

		if (oXML.nt("node"))
			var className = ".node-contents";
		else if (oXML.nt("input"))
			var className = ".input-values";
		return className;
	}
	...
	{
		var className = oXML.ntClass();
		return oXML.context.children(className).length;
	}
	oXML.changeLength = function()
	{
		oXML.length = oXML.count();
		return oXML;
	}
